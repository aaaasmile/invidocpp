...
void OptionGfx::Init(SDL_Rect* pRect, SDL_Surface*  pScreen, TTF_Font* pFontTxt, TTF_Font* pFontWinCtrl)
{
    ...
    rctBt1.x = m_pBtOK->m_rctButt.x ;
    m_pEditName->Init(&rctBt1, pScreen,  m_pFontText, MYEDITNAME);
    ...
    m_pComboLang->Init(&rctBt1, pScreen,  m_pFontText, MYIDCOMBOLAN);
...
void OptionGfx::Show(SDL_Surface* pScene_background)
{
    ...
    while (!m_bTerminated)
    {
        ...
        GFX_UTIL::DrawString(pShadowSrf, strNameTitle.c_str(), m_pEditName->m_rctButt.x , 
                    m_pEditName->m_rctButt.y - 20, GFX_UTIL_COLOR::Orange , m_pFontText);
        ...
        GFX_UTIL::DrawString(pShadowSrf, strLangSelectTitle.c_str(), m_pComboLang->m_rctButt.x , 
                    m_pComboLang->m_rctButt.y - 20, GFX_UTIL_COLOR::Orange , m_pFontText);
