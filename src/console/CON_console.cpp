...
//ISS define vsnprintf in windows
#ifdef _WINDOWS
...
void CON_UpdateConsole(ConsoleInformation *console) {
	...
	if(console->BackgroundImage) {
        ...
        if ( console->Back_rctSrc.x != -1 &&
             console->pSurf_Bar )
        {
            // ISS backround version
            DestRect.w = console->Back_rctSrc.w;
		    DestRect.h = console->Back_rctSrc.h;
		    SDL_BlitSurface(console->BackgroundImage, &console->Back_rctSrc, console->ConsoleSurface, &DestRect);
            GFX_UTIL::DrawStaticSpriteEx(console->ConsoleSurface, 0, 0, console->Back_rctSrc.w, console->Back_rctSrc.h, 
                                         0, 0, console->pSurf_Bar);
        }
        else
        {
...
ConsoleInformation *CON_Init(const char *FontName, SDL_Surface *DisplayScreen, int lines, SDL_Rect rect) {
    ...
    //ISS stuff
    newinfo->Back_rctSrc.x = -1;
    newinfo->Back_rctSrc.y = -1;
    newinfo->Back_rctSrc.w = -1;
    newinfo->Back_rctSrc.h = -1;
    newinfo->pSurf_Bar = 0;    
	...
	newinfo->FontHeight = TTF_FontHeight(g_pConsFontText);//DT_FontHeight(newinfo->FontNumber);
...
void DrawCommandLine() {
    ...
    if ( Topmost->Back_rctSrc.x != -1 && Topmost->pSurf_Bar )
    {
        // ISS backround version command line
        rect.x = 0;
        rect.y = Topmost->ConsoleSurface->h - Topmost->FontHeight;
	    rect.w = Topmost->InputBackground->w;
	    rect.h = Topmost->InputBackground->h;

        rect_srcLine.x = Topmost->Back_rctSrc.x;
        rect_srcLine.y = Topmost->Back_rctSrc.y + Topmost->Back_rctSrc.h -  Topmost->FontHeight;
        rect_srcLine.w = rect.w;
        rect_srcLine.h = rect.h;

        SDL_BlitSurface(Topmost->BackgroundImage, &rect_srcLine, Topmost->ConsoleSurface, &rect);
        
        GFX_UTIL::DrawStaticSpriteEx(Topmost->ConsoleSurface, 0, 0, 
                                     rect_srcLine.w, rect_srcLine.h, rect.x, rect.y, Topmost->pSurf_Bar);
    }
    else
    {
	    rect.x = 0;
	...
	if(Blink) 
    {
        ...
        int iPromptH;
        char buff[256];
        strcpy(buff, Topmost->Prompt);
        strcat(buff, Topmost->Command);
        TTF_SizeText(g_pConsFontText, buff, &iPromptW, &iPromptH);
        x = CON_CHAR_BORDER + iPromptW;
		if(Topmost->InsMode)
...
void CON_Out(ConsoleInformation *console, const char *str, ...) {
	...
	if(console->ConsoleLines) {
		while(strlen(ptemp) > (unsigned int)console->VChars) {
...
void CON_BackImages_Pres(ConsoleInformation *console, SDL_Texture* pImagBack, SDL_Rect& rctSrc, int iXDst, int iYDst)
{
    ...
    console->Back_rctSrc.x =  rctSrc.x;
    console->Back_rctSrc.y =  rctSrc.y;
    console->Back_rctSrc.w =  rctSrc.w;
    console->Back_rctSrc.h =  rctSrc.h;

    console->BackgroundImage = pImagBack;
...
void CON_SetPrompt(ConsoleInformation *console, char* newprompt) {
	...
	if(strlen(newprompt) < (unsigned int)console->VChars)
...
void Cursor_Right(ConsoleInformation *console) {
	...
	if((unsigned int)Topmost->CursorPos < strlen(Topmost->Command)) {
		Topmost->CursorPos++;
